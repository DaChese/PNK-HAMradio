...
function join(channel) {
  if (document.domain == 'hack.chat') {
    ...
    // Try /chat-ws first (matches upstream), fallback to root if needed
    const tryUrls = [
      `${proto}://${host}:6060/chat-ws`,
      `${proto}://${host}:6060/`
    ];
    ...
    const connect = () => {
      ...
      console.log('Hack.Chat: connecting to', url);
      ...
      ws.onopen = () => {
        ...
        if (location.hash) {
          ...
          const newNick = prompt('Nickname:', myNick);
          if (newNick !== null) myNick = newNick; else shouldConnect = false;
        }
      ...
      ws.onclose = () => {
        // if first URL failed immediately, try fallback once
        if (idx === 0) {
          idx = 1;
          connect();
          return;
      ...
      ws.onmessage = (message) => {
        ...
        if (command) command.call(null, args);
      };
    };
